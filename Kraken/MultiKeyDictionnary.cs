...
namespace Kraken
{
    ...
    internal class InterchangableBiKeyDictionnary<K, V> : Dictionary<Tuple<K, K>, V>
    {
        ...
        {
            bool ret = TryGetValue(Key(key1, key2), out value);
            if (ret)
                return ret;
            return TryGetValue(Key(key2, key1), out value);
        }
    ...
    internal class CycliqueTripleKeyDictionary<K,V> : Dictionary<Tuple<K,K,K>, V>
    {
        ...
        {
            bool ret = TryGetValue(Key(key1, key2, key3), out value);
            if (ret)
                return ret;
            ret = TryGetValue(Key(key2, key3, key1), out value);
            if (ret)
                return ret;
            return TryGetValue(Key(key3, key1, key2), out value);
        }
